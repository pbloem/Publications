...
\begin{definition}[Representations]
  ...
  and $m(y)=x$. Given a model class $\M$, define the set of all
  representations for $x$:
  \[
  R^\M(x):=\{(m,y):m\in\M, m(y)=x\}.
  \]
\end{definition}
...
\begin{definition}[Candidate set]
  The \emph{candidate models} for $x$ from model class $\M$ are
  \[
  C^\M_c(x):=\{m\in\M:L^m(x)\le L^\M(x)+c\}.
  \]
\end{definition}

\begin{definition}[Sophistication]
  The model complexity, sophistication, or facticity, of a string $x$
  given a model class $\M$ and a constant $c$ is defined by
  \[
  S^\M_c(x):=\min\{L(m):m\in C^\M_c(x)\}.
  \]
\end{definition}
...
\paragraph{Kolmogorov's Structure Function}
...
\paragraph{Koppel Sophistication}
...
\paragraph{Vit\'anyi's Sophistication / Meaningful Information}
...
\paragraph{Antunes's Coarse Sophistication}

\begin{itemize}
\item Model class is $\T$
\item Description method for models is unclear, but appears to be
  taken from Koppel, i.e. arbitrary but self-delimiting.
\item Candidate set is defined differently:
  $C^\T(x)=\{m^*\}$ where $m^*=\argmin_m L(m)+L^m(x)-C(x)$. Thus, there is no slack
  variable and $C^\T$ contains only a single model.
\end{itemize}

\paragraph{Adriaans' Facticity}


\begin{itemize}
\item Model class is $\C$
\item Description method for models is $L(m)=|\bar\imath|$ where
  $\phi_i=m$. The index function is assumed to be \emph{faithful},
  i.e. there is a constant $c$ such that $|i|-C(i)\le c|$ for all $i$.
\item Candidate set is $C^\C_0$.
