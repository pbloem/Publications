...
\]
Since $|\bar\imath|$ is a computable codelength function, there must be an $i_0$ such that $|\bar\imath|-K(i)>|v|$ for all $i>i_0$ (see~\cite[Theorem~2.3.1]{li1993introduction}). From this $i$ onwards, the second representation (using $v$) will have a shorter code length, so any two-part representation $\bar ip$ cannot achieve the minimum in the definition of the facticity if $i>i_0$. Consequently, the facticity is bounded by $F^\phi(x)<|\overline{\imath_0}|$ for all $x$.
\end{proof}

% idea 0: show collapse of structure function

% idea 1: ideal model selection is impossible
% procedure selects model m1
% there is another model m2 that dominates m1
% there is a numbering such that L(m1)+L_m1(data) > L(m2)+L_m2(data), 
% in the other numbering m2 is preferred
% limitation of this approach: you have no control over the difference in model complexity,
% it may not be very large.

% idea 2: model complexity is not invariant
% there are numberings \phi,\psi such that for all c there is an x such that |C_1(x)-C_2(x)|>=c
% prove this for total recursive functions?
% to make the result stronger:
% - show that it does not go away with penalties on model complexity
% - show that it holds for *all* numberings that are sufficiently distinct in some sense
% - show that it holds for more model classes
% - show that it occurs for most strings
%
% proof idea: truth is bernoulli with a complicated but computable parameter value, K(theta) is large
% for small x, the print function will outperform the true model.
% for large x, the true model gives best overall code length.
% somewhere in between, the model complexity jumps from the one to the other. But that will not happen
% at the same time for the two numberings. When one has switched and the other has not, the difference
% in model complexity is about K(theta), which can be made arbitrarily large.
% problem: what about in-between models that approximate theta?
