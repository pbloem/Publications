...
\section{Vitanyi's Sophistication}
 
\begin{lemma}
Given $n_0$ and $c_0$, there exist $y \in \B$ and $h \in \N$ such that 
\begin{itemize}
  \item $|0^hy| \geq n$,
  \item $|0^hy| - c \leq K(0^hy) \leq |0^hy| + c$,
  \item and $C(0^hy) + c' \geq K(h) + C(y)$ for some constant $c'$.
  \item $K(h) \geq c'' log(|0^hy|)$ for some positive $c''$
\end{itemize}
\end{lemma}

\noindent \pb{Ik denk dat we dit kunnen bewijzen, maar het leidt uiteindelijk maar tot een logterm variantie tussen verschillende versies van G. Dat betekent dat ze asymptotisch niet echt verschillen. Wat we nodig hebben is een groot model $f$ waarvan we weten dat er een $x$ in $B^n$ zit, die met $f$ $C(x)$ to op een constante benaderd. Een familie van modellen $f$ kan ook. Het probleem is dat het bij zulke lage compressie moeilijk is om een compressiemethode met een groot model te bedenken\ldots}

\begin{lemma}
There exists an acceptable numbering $\phi1, \phi2, \ldots$ such that for all $n_0$, $c_0$, there exist $x, n,c$ with $c > c_0$, $n> n_0$ and $x \in B^n_c$ so that $G^\phi(x) > c'\log(|x|)$ for some positive $c'$.
\end{lemma}


\begin{theorem}
The exist acceptable numberings $\phi_1, \phi_2, \ldots$ and $\psi_1, \psi_2, \ldots$ such that for any given constant $c$ there exist $x$ such that $|G_M^\phi(x) - G_M^\psi(x)| > c$
\end{theorem}
