...
\begin{theorem}
Let $A(n)$ be the single-argument Ackermann function and $c$ some arbitrary constant. For all strings with depth $d^\C_c(x) \leq A(|x|)$, $\s^\T(x)$, $s^\P(x)$ and $\s^\F$ are bounded for specific numberings.
\end{theorem}
\begin{proof}
\end{proof}
This shows that while high-sophistication strings exist for variants with limited model class, the mechanics do not follow the basic intuition of sophistication. Unless we encounter data that would take longer than the lifetime of the universe to produce (and then some), sophistication behaves exactly as it would without the restricted model class. 
...
\pb{Procedural view: we cannot choose between dominating models. This may not be the aim of algorithmic statistics, but it suggests that it's kind of unreasonable to ask for model selection without repeated samples, or assumptions on the model class.} 
