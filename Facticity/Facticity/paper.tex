...
K^\phi(f) &= \min\left\{ K^\phi(j) : \phi_j = f \right\}& \\
       &= \min\left\{ K^\phi(1^r0i) : \phi_{ri} = f \right\}& \\
       &\ge  \min\left\{ K^\phi(i) : \phi_{ri} = f \right\}-c'&\text{by~\eqref{eq:prelim1}}\\
       &\geq \min\left\{ K^\psi(i)  : \phi_{ri} = f \right\}+r+1-c'& \text{by \eqref{eq:prelim2}}\\
